...
class EAcmiFrameworkTol:
    ...
    def H_whiten(self, s):
        ...
        epsilon = 1e-6
        s_safe = np.where(s > 0, s, epsilon)
        H_w = np.sum(np.log(s_safe))
        return H_w
    ...
    def H_normal(self, X):
        ...
        X = np.atleast_2d(X)  # Ensure X is at least 2-D
        if not self.silent:
            print(f"H_normal input shape: {X.shape}")  # Controlled by silent

        d = X.shape[1]
        cov_X = np.cov(X, rowvar=False)

        if not self.silent:
            print(f"cov_X shape: {cov_X.shape}")  # Controlled by silent

        # Handle the case where covariance matrix is a scalar (single feature)
        if cov_X.ndim == 0:
            cov_X = np.array([[cov_X]])
            if not self.silent:
                print("Covariance matrix was scalar. Converted to 1x1 array.")
        ...
        try:
            ...
            np.linalg.cholesky(cov_X)
        except np.linalg.LinAlgError:
            # If not positive definite, add regularization
            cov_X += np.eye(cov_X.shape[0]) * reg
            if not self.silent:
                print("Covariance matrix was not positive definite. Added regularization.")

        # Compute determinant using log for numerical stability
        sign, logdet = np.linalg.slogdet(cov_X)
        if sign <= 0:
            if not self.silent:
                print("Non-positive determinant encountered after regularization. Setting determinant to small positive value.")
            logdet = np.log(1e-12)  # Assign a very small positive number to prevent log(0)

        H_w = 0.5 * logdet + (d / 2) * np.log(2 * np.pi) + (d / 2)
        return H_w
    ...
    def Edgeworth_t1_t2_t3(self, X):
        ...
        # Compute third and mixed moments
        # Vectorized computation for t1
        kappa_iii = np.nanmean(X ** 3, axis=0)
        t1 = np.sum(kappa_iii ** 2)

        # Vectorized computation for t2
        t2 = 0
        if d > 1:
            # Compute all pairwise (X_i^2 * X_j)
            X_squared = X ** 2
            for i, j in combinations(range(d), 2):
                kappa_iij = np.nanmean(X_squared[:, i] * X[:, j])
                t2 += kappa_iij ** 2
            t2 *= 6  # 3 * 2 * combination count

        # Vectorized computation for t3
        t3 = 0
        if d > 2:
            # Compute all triplet (X_i * X_j * X_k)
            for i, j, k in combinations(range(d), 3):
                kappa_ijk = np.nanmean(X[:, i] * X[:, j] * X[:, k])
                t3 += kappa_ijk ** 2
            t3 /= 6
    ...
    def H_EdgeworthApprox(self, X):
        ...
        X = np.atleast_2d(X)  # Ensure X is at least 2-D
        if not self.silent:
            print(f"H_EdgeworthApprox input shape: {X.shape}")  # Controlled by silent
        ...
        H_w = self.H_whiten(s)
        H_n = self.H_normal(Y)
        ea = self.Edgeworth_t1_t2_t3(Y)  # Corrected method name
        H_ea = (H_n - (ea['t1'] + ea['t2'] + ea['t3']) / 12) + H_w
        return H_ea
    ...
    def MI_EdgeworthApprox(self, Y, X):
        ...
        H_YX = self.H_EdgeworthApprox(np.hstack((Y.reshape(-1, 1), X)))
        MI_ea = H_Y + H_X - H_YX
        MI_ea = max(0, np.real(MI_ea))  # Ensure non-negative
        return MI_ea
    ...
    def CMI_EdgeworthApprox(self, Y, X, Z):
        ...
        H_YXZ = self.H_EdgeworthApprox(np.hstack((Y.reshape(-1, 1), X.reshape(-1, 1), Z)))
        CMI_ea = H_YZ + H_XZ - H_Z - H_YXZ
        CMI_ea = max(0, np.real(CMI_ea))  # Ensure non-negative
        return CMI_ea
    ...
    def select_features(self, x, y):
        ...
        """
        start_time = time.time()
        cpu_start = time.process_time()  # For CPU time
        if isinstance(x, pd.DataFrame):
            x = x.values
        if isinstance(y, pd.Series):
            y = y.values.reshape(-1, 1)
        ...
        # Generate input names starting from X1 to match R's 1-based indexing
        if isinstance(x, pd.DataFrame):
            inp_names = list(x.columns)
        else:
            inp_names = [f'X{i + 1}' for i in range(n_inputs)]
        ...
        input_tracker = list(range(n_inputs))  # 0-based indices
        n_selected = 0
        z_in = np.empty((n_data, 0))  # Initialize as empty 2D array
        scores = pd.DataFrame()
        max_iter = x_stand.shape[1] + 1

        # To track CMI_MI_ratio for stopping condition
        cmi_ratio_history = []

        for iter_1 in range(1, max_iter + 1):
            if not self.silent:
                print(f"\nIteration {iter_1}:")
                remaining_features = [inp_names[idx] for idx in input_tracker]
            ...
            if n_selected > 0:
                ...
                best_idx = np.argmax(CMI)
                best_CMI = CMI[best_idx]
                selected_input = input_tracker[best_idx]
                ...
                # Record CMI_MI_ratio history
                cmi_ratio_history.append(CMI_MI_ratio)
                ...
                score_entry = {
                    ...
                    'CMI_evals': nfevals_2,
                    'ElapsedTime': round(time.time() - start_time, 4),
                    'CPUtime': round(time.process_time() - cpu_start, 4)
                }
                scores = pd.concat([scores, pd.DataFrame([score_entry])], ignore_index=True)
                ...
                # Check stopping conditions
                if len(cmi_ratio_history) > 2 and (CMI_MI_ratio <= self.thresh):
                    if not self.silent:
                        print(f"CMI_MI_ratio ({CMI_MI_ratio}) <= thresh ({self.thresh}). Stopping selection.")
                    break

                if len(cmi_ratio_history) > 3:
                    # Check if the last three CMI_MI_ratios are increasing
                    if (cmi_ratio_history[-1] > cmi_ratio_history[-2] > cmi_ratio_history[-3]):
                        if not self.silent:
                            print("CMI_MI_ratio has been increasing for the last three iterations. Stopping selection.")
                        break

                # Remove selected input from tracker
                input_tracker.pop(best_idx)
                n_inputs -= 1
...
# Example Usage: The following is for testing (will be removed in the future)
